...
public:
  ...
  Decl *getDeclFromCall(CallExpr *CE) {
    if (FunctionDecl *CalleeDecl = CE->getDirectCallee())
      return CalleeDecl;
    CE->dump();
    //qali-add: also support funcPtr
    //todo: other funcPtr with no decl such as funcPtr[i]
    if(ValueDecl *CalleePtrDecl = dyn_cast_or_null<ValueDecl>(CE->getCalleeDecl()))
      return CalleePtrDecl;
  ...
  void VisitCallExpr(CallExpr *CE) {
    if (Decl *D = getDeclFromCall(CE))
      addCalledDecl(D, CE);
    //qali-add: deal with function-invocation with funcArr[](), where there is no Decl
    else
    {
      Expr *Ex = CE->getCallee();
      CallGraphNode *CalleeNode = new CallGraphNode(Ex);
...
CallGraphNode *CallGraph::getOrInsertNode(Decl *F) {
  ...
  // qali-update: skip function without body from roots' callee
  if (F && F->hasBody())
    Root->addCallee({Node.get(), /*Call=*/nullptr});
  return Node.get();
}

// insert node for callee from function array reference, no Decls
CallGraphNode *CallGraph::getOrInsertExprNode(Expr *E) {
  std::unique_ptr<CallGraphNode> &Node = ExprMap[E];
  if (Node)
    return Node.get();
  Node = std::make_unique<CallGraphNode>(E);
  return Node.get();
}
/*
...
void CallGraph::print(raw_ostream &OS) const {
  ...
  std::set<const CallGraphNode *> sortSet;
  ...
  for (llvm::ReversePostOrderTraversal<const CallGraph *>::rpo_iterator
    ...
    for (CallGraphNode::const_iterator CI = N->begin(),
      ...
      const CallGraphNode *child = *CI;
      if(sortSet.find(child) == sortSet.end())
        sortSet.insert(child);
      else
        continue;


      CI->Callee->print(OS);
      // dump the function type
      OS << "{";
      //const FunctionType *T = CI->Callee->getDecl()->getFunctionType();


       //= CI->CallExpr->getType();
      Decl *decl = child->getDecl();
      ...
      if(decl  )
      {
        ...
        assert(vDecl);
        SplitQualType T_split = vDecl->getType().split();
        OS << QualType::getAsString(T_split, child->getDecl()->getASTContext().getPrintingPolicy());
      }
      else
      {
        ASTContext &astCtx = N->getDecl()->getASTContext();
        SplitQualType T_split = child->getExpr()->getType().split();
        OS << QualType::getAsString(T_split, astCtx.getPrintingPolicy());
      }
...
void CallGraph::viewGraph(std::string moduleName) const {
  std::string name = moduleName + ".cg.dot";
  //qali-update
  llvm::WriteGraph(this, "", false,"",  name);
  //qali-end
}
